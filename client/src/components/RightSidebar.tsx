...
export function RightSidebar() {
  const { communityStats, bookClubs } = useCommunity();
  
  // Real data from context - no mock data
  const trendingTopics: any[] = []; // Will be populated when trending system is implemented
  const suggestedUsers: any[] = []; // Will be populated when user recommendation system is implemented

  return (
    <aside className="w-full xl:w-80 bg-card border-l border-border p-3 sm:p-4 lg:p-6 hidden lg:block" data-testid="right-sidebar">
              ...
              {bookClubs.slice(0, 3).map((club) => (
                <div key={club.id} className="bg-secondary/50 p-3 rounded-lg" data-testid={`book-club-${club.id}`}>
                  ...
                  <p className="text-xs text-muted-foreground" data-testid={`club-current-book-${club.id}`}>
                    Currently reading "{club.current_book?.title || club.current_book_title || 'No book selected'}"
                  </p>
                </div>
              ))}
            </div>
